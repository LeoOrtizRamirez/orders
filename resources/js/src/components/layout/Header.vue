            ...
            <div class="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-[#0e1726]">
                <div class="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2">
                    <router-link to="/" class="main-logo flex items-center shrink-0">
                        <img :class="'w-8 ltr:-ml-1 rtl:-mr-1 inline' + customClass" :src="logoPath" alt="" />                        
                    </router-link>
                ...
                >
                    ...
                    <div class="dropdown shrink-0">
                        <Popper :placement="store.rtlClass === 'rtl' ? 'bottom-end' : 'bottom-start'" offsetDistance="8" class="!block">
                            <button type="button" class="relative block p-2 rounded-full bg-white-light/40 dark:bg-dark/40 hover:text-primary hover:bg-white-light/90 dark:hover:bg-dark/60">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                ...
                                <span class="flex absolute w-3 h-3 ltr:right-0 rtl:left-0 top-0" v-if="notifications.length">
                                ...
                                <ul class="!py-0 text-dark dark:text-white-dark w-[300px] sm:w-[350px] divide-y dark:divide-white/10">
                                    <li>
                                        <div class="flex items-center px-4 py-2 justify-between">
                                            <h4 class="text-lg font-bold">Notificaciones</h4>
                                            <span v-if="notifications.length" class="badge bg-primary/80">{{ notifications.length }} Nuevas</span>
                                        </div>
                                    </li>
                        ...
                        <Popper :placement="store.rtlClass === 'rtl' ? 'bottom-end' : 'bottom-start'" offsetDistance="8" class="!block">
                            <button type="button" class="relative group block">
                                <img
                                    ...
                                    :src="authStore.user?.image ? '/storage/' + authStore.user.image : '/assets/images/profile-34.jpeg'"
                                    alt=""
                                />
                            </button>
                            <template #content="{ close }">
                                <ul class="text-dark dark:text-white-dark !py-0 w-[230px] font-semibold dark:text-white-light/90">
                                    <li>
                                        <div class="flex items-center px-4 py-4">
                                            <div class="flex-none">
                                                <img class="rounded-md w-10 h-10 object-cover" :src="authStore.user?.image ? '/storage/' + authStore.user.image : '/assets/images/profile-34.jpeg'" alt="" />
                                            </div>
                                            <div class="ltr:pl-4 rtl:pr-4 truncate">
                                                <h4 class="text-base">
                                                    {{ authStore.user?.name || 'Usuario' }}<span class="text-xs bg-success-light rounded text-success px-1 ltr:ml-2 rtl:ml-2">Pro</span>
                                                </h4>
                                    ...
                                    <li>
                                        <router-link to="/users/user-account-settings" class="dark:hover:text-white" @click="close()">
                                            <svg
                                    ...
                                    <li class="border-t border-white-light dark:border-white-light/10">
                                        <a href="javascript:;" class="text-danger !py-3" @click="handleLogout(close)">
                                            <svg
...
<script lang="ts" setup>
    ...
    const markAllAsRead = async (closeDropdown: Function) => {
        try {
    ...
    const formatNotificationTime = (timestamp: string) => {
        ...
        const diffSeconds = Math.round((now.getTime() - notificationDate.getTime()) / 1000);
    ...
    const handleLogout = async (closeDropdown: () => void) => {
        try {
