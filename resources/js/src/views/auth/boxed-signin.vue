<template>
    <div>
        ...
        <div
            :class="'relative flex min-h-screen items-center justify-center bg-[url(/assets/images/'+getImageTheme+')] bg-cover bg-center bg-no-repeat px-6 py-10 dark:bg-[#060818] sm:px-16'"
        >
            <div
            ...
            >
                <div class="relative flex flex-col justify-center rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 px-6 lg:min-h-[758px] py-20">
                    ...
                    <div class="mx-auto w-full max-w-[440px]">
                        <div class="mb-10">
                            <h1 class="text-3xl font-extrabold uppercase !leading-snug text-primary md:text-4xl">{{ $t('login_page.sign_in') }}</h1>
                            <p class="text-base font-bold leading-normal text-white-dark">{{ $t('login_page.desc') }}</p>
                        </div>
                        
                        <!-- Mensajes de estado -->
                        <div v-if="errorMessage" class="mb-4 bg-red-500/10 border border-red-500 text-red-700 px-4 py-3 rounded relative" role="alert">
                            <span class="block sm:inline">{{ errorMessage }}</span>
                        </div>
                        
                        <div v-if="successMessage" class="mb-4 bg-green-500/10 border border-green-500 text-green-700 px-4 py-3 rounded relative" role="alert">
                        ...
                        <form class="space-y-5 dark:text-white" @submit.prevent="handleLogin">
                            ...
                            <!-- <div class="flex items-center justify-between">
                                <router-link 
                                    ...
                                    class="text-primary underline text-sm hover:text-black dark:hover:text-white"
                                >
                            ...
                            >
                                <span v-if="loading">
                                    ...
                                    {{ $t('login_page.btn_send') }}
                                </span>
                                <span v-else>{{ $t('login_page.btn') }}</span>
                            </button>
                        </form>
...
<script lang="ts" setup>
    ...
    const getImageTheme = computed(() => {
        return store.theme == 'dark' ? 'bg.webp' : 'bg-white.webp';
    });
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            // Limpiar autenticación en caso de error
            authStore.logout();
        } finally {
            loading.value = false;
        }
    ...
    // Limpiar mensajes cuando se cambian los campos
    const clearErrors = () => {
        errorMessage.value = '';
        errors.value = {};
    };
